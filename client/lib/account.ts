...
export const saveAccount = async (user: AuthUser, account: AccountData) => {
  ...
  const response = await fetch("/api/account", { method: "PUT", headers: await headersFor(user), body: JSON.stringify(account) });
  if (!response.ok) throw new Error("Unable to save account changes.");
  return account;
};

export const savePassword = async (_user: AuthUser, _currentPassword: string, newPassword: string) => {
  if (!supabase) throw new Error("Supabase Authentication is not configured for this storefront.");
