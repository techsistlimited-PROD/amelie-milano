...
const ShoeCard = ({ product }: { product: (typeof shoeProducts)[number] }) => {
  const { saved, toggle } = useWishlist({ id: product.id, name: product.name, price: product.salePrice ?? product.price, image: product.image, category: "Shoes", colour: "Signature colour", option: "M" });
  const [quantity, setQuantity] = useState(1);
  const [added, setAdded] = useState(false);
  const handleAdd = () => { addToCart({ id: product.id, name: product.name, price: product.salePrice ?? product.price, image: product.image, category: "Shoes", colour: "Signature colour", option: "M", quantity }); setAdded(true); };
